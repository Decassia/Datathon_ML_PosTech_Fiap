...
const Previsoes = () => {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(`Erro ${response.status}: ${rawText}`);
        return;
      }

      const data = JSON.parse(rawText);
      const inde = Number(data.inde);
      ...
      setError("Não foi possível realizar a previsão. Verifique a conexão com a API.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-8">
      ...
      <Tabs defaultValue="geral">
        <TabsList className="bg-muted p-1 rounded-xl">
          ...
          <TabsTrigger value="detalhada" className="rounded-lg">
            Previsão Detalhada
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="geral" className="mt-6">
          <div className="bg-card rounded-xl p-8 shadow-card space-y-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h3 className="font-heading font-semibold text-foreground text-lg">
                  Modelo de Previsão do INDE
                </h3>
                <p className="text-muted-foreground mt-2 leading-relaxed">
                  Desenvolvido para auxiliar no desenvolvimento dos alunos, esse
                  modelo prevê, antes do final do ano letivo, o INDE (Índice de
                  Desenvolvimento Educacional) de um aluno. A previsão é feita com
                  apenas três dos sete indicadores utilizados na ponderação do INDE
                  e algumas informações do aluno.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4 text-sm text-muted-foreground">
              ...
              <p>
                Pode ser disponibilizado para professores ou alunos, permitindo
                análise eficiente do progresso e motivação para melhorias.
              </p>
              <p className="text-xs italic">
                Nota: não realiza previsões para alunos da fase 8.
              </p>
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="detalhada" className="mt-6">
          <div className="bg-card rounded-xl p-8 shadow-card space-y-6">
            ...
            <Button
              ...
              disabled={loading}
              className="gradient-primary text-primary-foreground px-8 py-3 text-base font-heading font-semibold rounded-xl hover:opacity-90 transition-opacity"
            >
            ...
            {result && (
              <div
                className={`rounded-xl p-6 bg-gradient-to-r ${pedraColor[result.pedra]} text-primary-foreground animate-slide-up`}
              >
                <h4 className="text-2xl font-heading font-bold">{result.pedra}!</h4>
                <p className="text-lg mt-1">
                  INDE previsto: <strong>{result.inde.toFixed(2)}</strong>
                </p>
              </div>
            )}
          </div>
